...
#define kOutputFileName "DrumComp.wav"
#define kThresh 0.2f
...
int main(void){
  ...
  gCompRatio = 1.0f / 4.0f;
  gGain = 0.8f / (kThresh + (1.0f - kThresh));
...
void process(float *inBuffer, float *outBuffer, sf_count_t bufferSize){
  ...
  for(sf_count_t n = 0; n < bufferSize; n++){
      ...
      if(outBuffer[n] > kThresh){
          //Compresss
          outBuffer[n] = kThresh + (outBuffer[n] - kThresh * gCompRatio);
      } else if (outBuffer[n] < -kThresh){
          outBuffer[n] = -kThresh + (outBuffer[n] + kThresh * gCompRatio);
      }
      outBuffer[n] *= gGain;
  }
